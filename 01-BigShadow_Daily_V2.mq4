...
NAME: BIG SHADOW
SELL RULES:
   1. The close of first candle should be lower than second one.(Close1 <= Close2)
   2. The body should be larger than 3 of 5 candles.
   3. It must have a lower low.Eg. (Low1 <= Low2)
   4. Weak low should represent 30% percent of candle AT MAXIMUM.
   5. The previous 4 candles should not close above the first candle open.
   6. The first candle should be bearish.
   7. The second candle should be bullish.
BUY RULES:
...
extern double  BodyMinimalSize=500;
...
//+------------------------------------------------------------------+
bool _Condition_isCloseLowerThanPrevious;
bool _Condition_isWeakLowAcceptable;
bool _Condition_isCandleBodyLargerThanPrevious;
...
bool _Condition_isBodyBigEnough;
...
_Candle_WeakLow = NormalizeDouble(((_Bars_Close[0] - _Bars_Low[0]) * 100)/(_Candle_TotalSizePrice <= 0 ? 1: _Candle_TotalSizePrice),Digits);
_Candle_WeakHigh = NormalizeDouble((_Bars_High[0] - _Bars_Close[0] * 100)/(_Candle_TotalSizePrice <= 0 ? 1: _Candle_TotalSizePrice),Digits);
...
bool IsBuySignal(){
   if(IsBullishCandle(_Bars_Open[0],_Bars_Close[0])){
      //# BUY RULE 01 : The close of first candle should be higher than second one.(Close1 <= Close2)
      _Condition_isCloseLowerThanPrevious = (_Bars_Close[0] +(50*Point)) >= _Bars_Open[1];
      
      //# BUY RULE 02 : The body of first candle should be larger than the previous 5 candles
      _Condition_isCandleBodyLargerThanPrevious = true;
      int _totalBarsLargerThanPrevious = 0;
      for(int i=2; i<7;i++){
         if(_Candle_BodySize < GetCandleBodySize(_Bars_Open[i],_Bars_Close[i])){
            _totalBarsLargerThanPrevious++;
         }
      }
      if(_totalBarsLargerThanPrevious > 2)
         _Condition_isCandleBodyLargerThanPrevious = false;
         
      //# BUY RULE 03: It must have a higher high
      _Condition_hasHigherHighAndLowerLow = _Bars_High[0] >= _Bars_High[1];
      
      //# BUY RULE 04: Weak high should represent 0 to 30% percent of candle AT MAXIMUM.
      _Condition_isWeakLowAcceptable = _Candle_WeakHigh <= 30;//((barWeakMax/100) * _candleSize);
      
      //# BUY RULE 05: The previous 5 candles should not close above the first candle open.
      _Condition_hasSpaceToTheLeft = true;
      for(int j=2; j<5; j++){
         if(_Bars_Open[j] <= _Bars_Open[0] || _Bars_Close[j] <= _Bars_Open[0] || _Bars_Low[j] <= _Bars_Open[0]){
            _Condition_hasSpaceToTheLeft = false;
            break;
         }
      }

      //# BUY RULE 06: The first candle should be bullish
      _Condition_isFirstCandleBullish = IsBullishCandle(_Bars_Open[0],_Bars_Close[0]);
      
      //# BUY RULE 07: The second candle should be bearish
      _Condition_isSecondCandleBearish = IsBearishCandle(_Bars_Open[1],_Bars_Close[1]);
      ...
      return _Condition_isCloseLowerThanPrevious &&
             ...
             _Condition_isCandleBodyLargerThanPrevious &&
             _Condition_hasSpaceToTheLeft &&
             _Condition_isFirstCandleBullish &&
             _Condition_isBodyBigEnough &&
             _Condition_isSecondCandleBearish;
   }
   else{
      return false;
   }
...
bool IsSellSignal(){
      //# SELL RULE 01 : The close of first candle should be lower than second one.(Close1 <= Close2)
      _Condition_isCloseLowerThanPrevious = (_Bars_Close[0] -(50*Point)) <= _Bars_Open[1];
      
      //# SELL RULE 02 : The body of first candle should be larger than the previous 5 candles
      _Condition_isCandleBodyLargerThanPrevious = true;
      int _totalBarsLargerThanPrevious = 0;
      for(int i=2; i<7;i++){
         if(_Candle_BodySize < GetCandleBodySize(_Bars_Open[i],_Bars_Close[i])){
      ...
      //# SELL RULE 03: It must have a lower low
      _Condition_hasHigherHighAndLowerLow = _Bars_Low[0] <= _Bars_Low[1];
       
      //# SELL RULE 04: Weak low should represent 0 to 25% percent of candle AT MAXIMUM.
      _Condition_isWeakLowAcceptable = _Candle_WeakLow <= 30;//((barWeakMax/100) * _candleSize);
      
      //# SELL RULE 05: The previous 5 candles should not close above the first candle open.
      _Condition_hasSpaceToTheLeft = true;
      for(int j=2; j<5; j++){
         if(_Bars_Open[j] > _Bars_Open[0] || _Bars_Close[j] > _Bars_Open[0] || _Bars_High[j] > _Bars_Open[0]){
            _Condition_hasSpaceToTheLeft = false;
            break;
         }
      }
      
      //# SELL RULE 06: The first candle should be bearish
      _Condition_isFirstCandleBearish = IsBearishCandle(_Bars_Open[0],_Bars_Close[0]);
      
      //# SELL RULE 07: The second candle should be bullish
      ...
      return _Condition_isCloseLowerThanPrevious &&
             ...
             _Condition_isCandleBodyLargerThanPrevious &&
             _Condition_hasSpaceToTheLeft &&
             _Condition_isFirstCandleBearish &&
             _Condition_isBodyBigEnough &&
             _Condition_isSecondCandleBullish;
}
